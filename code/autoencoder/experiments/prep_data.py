...
def stack_volumes(volumes):
    ...
    out = torch.stack(list_of_volumes, 1)
...
def get_normalized_pyrad(sub_ids, extractor, transforms, cubify):
    all_t1c_features = []
    all_t2f_features = []
    ...
    for sub_id in sub_ids:
        ...
        t1c_tensor = torch.tensor(t1c, dtype=torch.float32)
        t2f_tensor = torch.tensor(t2f, dtype=torch.float32)
        ...
        pyrad_sample = transforms(cubify(pyrad_sample))

        t1c_sitk = sitk.GetImageFromArray(pyrad_sample['mris']['t1c'])
        t2f_sitk = sitk.GetImageFromArray(pyrad_sample['mris']['t2f'])
        seg_sitk = sitk.GetImageFromArray(pyrad_sample['segs'][22])
...
class UnlabeledScansDataset(Dataset):
    def __init__(self, root_dir, size, transforms = None, output_dir = 'data/pytorch_datasets', use_pyrad = False):
        ...
        self.use_pyrad = use_pyrad
        #self.normalized_t1c_pyrads, self.normalized_t2f_pyrads = get_normalized_pyrad(self.sub_ids, self.extractor, self.transforms, self.cubify)
    ...
    def __getitem__(self, idx):
        ...
        if os.path.exists(cache_path):
            ...
            if self.use_pyrad:
                ...
                seg = torch.tensor(seg, dtype=torch.int)
                seg_bin = torch.tensor(seg_bin, dtype=torch.int)
                t1c = torch.tensor(t1c, dtype=torch.float32)
                t2f = torch.tensor(t2f, dtype=torch.float32)
                ...
                for seg_type in sample['segs']:
                    sample['segs'][seg_type] = sample['segs'][seg_type].long()
